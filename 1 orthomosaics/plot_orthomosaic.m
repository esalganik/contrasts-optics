...
x_dark = 107;                 % x-position of open-water reference point [m]
y_dark = 188;                % y-position of open-water reference point [m]
albedo_dark = 0.07;

x_bright = 183.6;              % x-position of bright ice reference point [m]
y_bright = 135.7;              % y-position of bright ice reference point [m]
albedo_bright = 0.70;
...
[~,j_dark] = min(abs(y - y_dark));

[~,i_bright] = min(abs(x - x_bright));
[~,j_bright] = min(abs(y - y_bright));

B_dark = mean(double(RGB(j_dark,i_dark,1:3)));
B_bright = mean(double(RGB(j_bright,i_bright,1:3)));
...
plot(x_dark,y_dark,'co','MarkerSize',5,'LineWidth',1.2)
text(x_dark,y_dark,'  open-water ref','Color','c','FontSize',7)

plot(x_bright,y_bright,'mo','MarkerSize',5,'LineWidth',1.2)
text(x_bright,y_bright,'  bright-ice ref','Color','m','FontSize',7)
